...
def smart_game_test(board, red, black):
    """
...
def run_multiple_games(games, board_size):
    '''
    ...
    for i in range(games):
        ...
        if i % 2 == 0: #smart is black
            ...
            smart = "RED"
        board_record.append(((smart_game_test(board, red, black), smart)))
        board.reset_board()
        i += 1

matches = run_multiple_games(100,3) #edit values for preferred (games, board size)
...
def win_pct(board_record):
    ...
    for match in board_record:
        ...
        if winner == smart_color:
            smart_wins += 1
    return smart_wins/games

def test_win_pct():
    """
    Asserts smart bot wins more than 50% of games.
    ...
    Return:
        Nonw
    """
    assert win_pct(board_record) >= .5

print(win_pct()) #just to show its precise percentage
